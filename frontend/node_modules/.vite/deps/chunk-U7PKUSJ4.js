import {
  __commonJS
} from "./chunk-EQCVQC35.js";

// node_modules/@bangladeshi/bangladesh-address/build/src/json/bd-upazila.json
var require_bd_upazila = __commonJS({
  "node_modules/@bangladeshi/bangladesh-address/build/src/json/bd-upazila.json"(exports, module) {
    module.exports = [
      {
        upazila: "Amtali",
        district: "Barguna",
        division: "Barisal"
      },
      {
        upazila: "Bamna",
        district: "Barguna",
        division: "Barisal"
      },
      {
        upazila: "Barguna Sadar",
        district: "Barguna",
        division: "Barisal"
      },
      {
        upazila: "Betagi",
        district: "Barguna",
        division: "Barisal"
      },
      {
        upazila: "Patharghata",
        district: "Barguna",
        division: "Barisal"
      },
      {
        upazila: "Taltali",
        district: "Barguna",
        division: "Barisal"
      },
      {
        upazila: "Muladi",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Babuganj",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Agailjhara",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Barisal Sadar",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Bakerganj",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Banaripara",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Gaurnadi",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Hizla",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Mehendiganj",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Wazirpur",
        district: "Barisal",
        division: "Barisal"
      },
      {
        upazila: "Bhola Sadar",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Burhanuddin",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Char Fasson",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Daulatkhan",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Lalmohan",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Manpura",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Tazumuddin",
        district: "Bhola",
        division: "Barisal"
      },
      {
        upazila: "Jhalokati Sadar",
        district: "Jhalokati",
        division: "Barisal"
      },
      {
        upazila: "Kathalia",
        district: "Jhalokati",
        division: "Barisal"
      },
      {
        upazila: "Nalchity",
        district: "Jhalokati",
        division: "Barisal"
      },
      {
        upazila: "Rajapur",
        district: "Jhalokati",
        division: "Barisal"
      },
      {
        upazila: "Bauphal",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Dashmina",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Galachipa",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Kalapara",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Mirzaganj",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Patuakhali Sadar",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Dumki",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Rangabali",
        district: "Patuakhali",
        division: "Barisal"
      },
      {
        upazila: "Bhandaria",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Kaukhali",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Mathbaria",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Nazirpur",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Nesarabad",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Pirojpur Sadar",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Zianagar",
        district: "Pirojpur",
        division: "Barisal"
      },
      {
        upazila: "Bandarban Sadar",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Thanchi",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Lama",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Naikhongchhari",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Ali kadam",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Rowangchhari",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Ruma",
        district: "Bandarban",
        division: "Chattogram"
      },
      {
        upazila: "Brahmanbaria Sadar",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Ashuganj",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Nasirnagar",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Nabinagar",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Sarail",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Shahbazpur Town",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Kasba",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Akhaura",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Bancharampur",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Bijoynagar",
        district: "Brahmanbaria",
        division: "Chattogram"
      },
      {
        upazila: "Chandpur Sadar",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Faridganj",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Haimchar",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Haziganj",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Kachua",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Matlab Uttar",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Matlab Dakkhin",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Shahrasti",
        district: "Chandpur",
        division: "Chattogram"
      },
      {
        upazila: "Anwara",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Banshkhali",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Boalkhali",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Chandanaish",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Fatikchhari",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Hathazari",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Lohagara",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Mirsharai",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Patiya",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Rangunia",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Raozan",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Sandwip",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Satkania",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Sitakunda",
        district: "Chattogram",
        division: "Chattogram"
      },
      {
        upazila: "Barura",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Brahmanpara",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Burichong",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Chandina",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Chauddagram",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Daudkandi",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Debidwar",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Homna",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Comilla Sadar",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Laksam",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Monohorgonj",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Meghna",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Muradnagar",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Nangalkot",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Comilla Sadar South",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Titas",
        district: "Comilla",
        division: "Chattogram"
      },
      {
        upazila: "Chakaria",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Cox's Bazar Sadar",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Kutubdia",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Maheshkhali",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Ramu",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Teknaf",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Ukhia",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Pekua",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Feni Sadar",
        district: "Feni",
        division: "Chattogram"
      },
      {
        upazila: "Chagalnaiya",
        district: "Feni",
        division: "Chattogram"
      },
      {
        upazila: "Daganbhyan",
        district: "Feni",
        division: "Chattogram"
      },
      {
        upazila: "Parshuram",
        district: "Feni",
        division: "Chattogram"
      },
      {
        upazila: "Fhulgazi",
        district: "Feni",
        division: "Chattogram"
      },
      {
        upazila: "Sonagazi",
        district: "Feni",
        division: "Chattogram"
      },
      {
        upazila: "Dighinala",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Khagrachhari",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Lakshmichhari",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Mahalchhari",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Manikchhari",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Matiranga",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Panchhari",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Ramgarh",
        district: "Khagrachari",
        division: "Chattogram"
      },
      {
        upazila: "Lakshmipur Sadar",
        district: "Lakshmipur",
        division: "Chattogram"
      },
      {
        upazila: "Raipur",
        district: "Lakshmipur",
        division: "Chattogram"
      },
      {
        upazila: "Ramganj",
        district: "Lakshmipur",
        division: "Chattogram"
      },
      {
        upazila: "Ramgati",
        district: "Lakshmipur",
        division: "Chattogram"
      },
      {
        upazila: "Komol Nagar",
        district: "Lakshmipur",
        division: "Chattogram"
      },
      {
        upazila: "Noakhali Sadar",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Begumganj",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Chatkhil",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Companyganj",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Shenbag",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Hatia",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Kobirhat",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Sonaimuri",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Suborno Char",
        district: "Noakhali",
        division: "Chattogram"
      },
      {
        upazila: "Rangamati Sadar",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Belaichhari",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Bagaichhari",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Barkal",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Juraichhari",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Rajasthali",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Kaptai",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Langadu",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Nannerchar",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Kaukhali",
        district: "Rangamati",
        division: "Chattogram"
      },
      {
        upazila: "Dhamrai",
        district: "Dhaka",
        division: "Dhaka"
      },
      {
        upazila: "Dohar",
        district: "Dhaka",
        division: "Dhaka"
      },
      {
        upazila: "Keraniganj",
        district: "Dhaka",
        division: "Dhaka"
      },
      {
        upazila: "Nawabganj",
        district: "Dhaka",
        division: "Dhaka"
      },
      {
        upazila: "Savar",
        district: "Dhaka",
        division: "Dhaka"
      },
      {
        upazila: "Faridpur Sadar",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Boalmari",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Alfadanga",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Madhukhali",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Bhanga",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Nagarkanda",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Charbhadrasan",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Sadarpur",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Shaltha",
        district: "Faridpur",
        division: "Dhaka"
      },
      {
        upazila: "Gazipur Sadar-Joydebpur",
        district: "Gazipur",
        division: "Dhaka"
      },
      {
        upazila: "Kaliakior",
        district: "Gazipur",
        division: "Dhaka"
      },
      {
        upazila: "Kapasia",
        district: "Gazipur",
        division: "Dhaka"
      },
      {
        upazila: "Sripur",
        district: "Gazipur",
        division: "Dhaka"
      },
      {
        upazila: "Kaliganj",
        district: "Gazipur",
        division: "Dhaka"
      },
      {
        upazila: "Tongi",
        district: "Gazipur",
        division: "Dhaka"
      },
      {
        upazila: "Gopalganj Sadar",
        district: "Gopalganj",
        division: "Dhaka"
      },
      {
        upazila: "Kashiani",
        district: "Gopalganj",
        division: "Dhaka"
      },
      {
        upazila: "Kotalipara",
        district: "Gopalganj",
        division: "Dhaka"
      },
      {
        upazila: "Muksudpur",
        district: "Gopalganj",
        division: "Dhaka"
      },
      {
        upazila: "Tungipara",
        district: "Gopalganj",
        division: "Dhaka"
      },
      {
        upazila: "Dewanganj",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Baksiganj",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Islampur",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Jamalpur Sadar",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Madarganj",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Melandaha",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Sarishabari",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Narundi Police I.C",
        district: "Jamalpur",
        division: "Mymensingh"
      },
      {
        upazila: "Astagram",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Bajitpur",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Bhairab",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Hossainpur",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Itna",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Karimganj",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Katiadi",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Kishoreganj Sadar",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Kuliarchar",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Mithamain",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Nikli",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Pakundia",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Tarail",
        district: "Kishoreganj",
        division: "Dhaka"
      },
      {
        upazila: "Madaripur Sadar",
        district: "Madaripur",
        division: "Dhaka"
      },
      {
        upazila: "Kalkini",
        district: "Madaripur",
        division: "Dhaka"
      },
      {
        upazila: "Rajoir",
        district: "Madaripur",
        division: "Dhaka"
      },
      {
        upazila: "Shibchar",
        district: "Madaripur",
        division: "Dhaka"
      },
      {
        upazila: "Dasar",
        district: "Madaripur",
        division: "Dhaka"
      },
      {
        upazila: "Manikganj Sadar",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Singair",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Shibalaya",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Saturia",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Harirampur",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Ghior",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Daulatpur",
        district: "Manikganj",
        division: "Dhaka"
      },
      {
        upazila: "Lohajang",
        district: "Munshiganj",
        division: "Dhaka"
      },
      {
        upazila: "Sreenagar",
        district: "Munshiganj",
        division: "Dhaka"
      },
      {
        upazila: "Munshiganj Sadar",
        district: "Munshiganj",
        division: "Dhaka"
      },
      {
        upazila: "Sirajdikhan",
        district: "Munshiganj",
        division: "Dhaka"
      },
      {
        upazila: "Tongibari",
        district: "Munshiganj",
        division: "Dhaka"
      },
      {
        upazila: "Gazaria",
        district: "Munshiganj",
        division: "Dhaka"
      },
      {
        upazila: "Bhaluka",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Trishal",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Haluaghat",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Muktagachha",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Dhobaura",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Fulbaria",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Gaffargaon",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Gauripur",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Ishwarganj",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Mymensingh Sadar",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Nandail",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Phulpur",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Tarakanda",
        district: "Mymensingh",
        division: "Mymensingh"
      },
      {
        upazila: "Araihazar",
        district: "Narayanganj",
        division: "Dhaka"
      },
      {
        upazila: "Sonargaon",
        district: "Narayanganj",
        division: "Dhaka"
      },
      {
        upazila: "Bandar",
        district: "Narayanganj",
        division: "Dhaka"
      },
      {
        upazila: "Naryanganj Sadar",
        district: "Narayanganj",
        division: "Dhaka"
      },
      {
        upazila: "Rupganj",
        district: "Narayanganj",
        division: "Dhaka"
      },
      {
        upazila: "Siddirgonj",
        district: "Narayanganj",
        division: "Dhaka"
      },
      {
        upazila: "Belabo",
        district: "Narsingdi",
        division: "Dhaka"
      },
      {
        upazila: "Monohardi",
        district: "Narsingdi",
        division: "Dhaka"
      },
      {
        upazila: "Narsingdi Sadar",
        district: "Narsingdi",
        division: "Dhaka"
      },
      {
        upazila: "Palash",
        district: "Narsingdi",
        division: "Dhaka"
      },
      {
        upazila: "Raipura",
        district: "Narsingdi",
        division: "Dhaka"
      },
      {
        upazila: "Shibpur",
        district: "Narsingdi",
        division: "Dhaka"
      },
      {
        upazila: "Kendua",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Atpara",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Barhatta",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Durgapur",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Kalmakanda",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Madan",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Mohanganj",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Netrokona Sadar",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Purbadhala",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Khaliajuri",
        district: "Netrokona",
        division: "Mymensingh"
      },
      {
        upazila: "Baliakandi",
        district: "Rajbari",
        division: "Dhaka"
      },
      {
        upazila: "Goalandaghat",
        district: "Rajbari",
        division: "Dhaka"
      },
      {
        upazila: "Pangsha",
        district: "Rajbari",
        division: "Dhaka"
      },
      {
        upazila: "Kalukhali",
        district: "Rajbari",
        division: "Dhaka"
      },
      {
        upazila: "Rajbari Sadar",
        district: "Rajbari",
        division: "Dhaka"
      },
      {
        upazila: "Shariatpur Sadar",
        district: "Shariatpur",
        division: "Dhaka"
      },
      {
        upazila: "Damudya",
        district: "Shariatpur",
        division: "Dhaka"
      },
      {
        upazila: "Naria",
        district: "Shariatpur",
        division: "Dhaka"
      },
      {
        upazila: "Jajira",
        district: "Shariatpur",
        division: "Dhaka"
      },
      {
        upazila: "Bhedarganj",
        district: "Shariatpur",
        division: "Dhaka"
      },
      {
        upazila: "Gosairhat",
        district: "Shariatpur",
        division: "Dhaka"
      },
      {
        upazila: "Jhenaigati",
        district: "Sherpur",
        division: "Mymensingh"
      },
      {
        upazila: "Nakla",
        district: "Sherpur",
        division: "Mymensingh"
      },
      {
        upazila: "Nalitabari",
        district: "Sherpur",
        division: "Mymensingh"
      },
      {
        upazila: "Sherpur Sadar",
        district: "Sherpur",
        division: "Mymensingh"
      },
      {
        upazila: "Sreebardi",
        district: "Sherpur",
        division: "Mymensingh"
      },
      {
        upazila: "Tangail Sadar",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Sakhipur",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Basail",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Madhupur",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Ghatail",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Kalihati",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Nagarpur",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Mirzapur",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Gopalpur",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Delduar",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Bhuapur",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Dhanbari",
        district: "Tangail",
        division: "Dhaka"
      },
      {
        upazila: "Bagerhat Sadar",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Chitalmari",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Fakirhat",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Kachua",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Mollahat",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Mongla",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Morrelganj",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Rampal",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Sarankhola",
        district: "Bagerhat",
        division: "Khulna"
      },
      {
        upazila: "Damurhuda",
        district: "Chuadanga",
        division: "Khulna"
      },
      {
        upazila: "Chuadanga Sadar",
        district: "Chuadanga",
        division: "Khulna"
      },
      {
        upazila: "Jibannagar",
        district: "Chuadanga",
        division: "Khulna"
      },
      {
        upazila: "Alamdanga",
        district: "Chuadanga",
        division: "Khulna"
      },
      {
        upazila: "Abhaynagar",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Keshabpur",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Bagherpara",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Jessore Sadar",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Chaugachha",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Manirampur",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Jhikargachha",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Sharsha",
        district: "Jessore",
        division: "Khulna"
      },
      {
        upazila: "Jhenaidah Sadar",
        district: "Jhenaidah",
        division: "Khulna"
      },
      {
        upazila: "Maheshpur",
        district: "Jhenaidah",
        division: "Khulna"
      },
      {
        upazila: "Kaliganj",
        district: "Jhenaidah",
        division: "Khulna"
      },
      {
        upazila: "Kotchandpur",
        district: "Jhenaidah",
        division: "Khulna"
      },
      {
        upazila: "Shailkupa",
        district: "Jhenaidah",
        division: "Khulna"
      },
      {
        upazila: "Harinakunda",
        district: "Jhenaidah",
        division: "Khulna"
      },
      {
        upazila: "Terokhada",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Batiaghata",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Dacope",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Dumuria",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Dighalia",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Koyra",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Paikgachha",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Phultala",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Rupsa",
        district: "Khulna",
        division: "Khulna"
      },
      {
        upazila: "Kushtia Sadar",
        district: "Kushtia",
        division: "Khulna"
      },
      {
        upazila: "Kumarkhali",
        district: "Kushtia",
        division: "Khulna"
      },
      {
        upazila: "Daulatpur",
        district: "Kushtia",
        division: "Khulna"
      },
      {
        upazila: "Mirpur",
        district: "Kushtia",
        division: "Khulna"
      },
      {
        upazila: "Bheramara",
        district: "Kushtia",
        division: "Khulna"
      },
      {
        upazila: "Khoksa",
        district: "Kushtia",
        division: "Khulna"
      },
      {
        upazila: "Magura Sadar",
        district: "Magura",
        division: "Khulna"
      },
      {
        upazila: "Mohammadpur",
        district: "Magura",
        division: "Khulna"
      },
      {
        upazila: "Shalikha",
        district: "Magura",
        division: "Khulna"
      },
      {
        upazila: "Sreepur",
        district: "Magura",
        division: "Khulna"
      },
      {
        upazila: "Gangni",
        district: "Meherpur",
        division: "Khulna"
      },
      {
        upazila: "Mujibnagar",
        district: "Meherpur",
        division: "Khulna"
      },
      {
        upazila: "Meherpur Sadar",
        district: "Meherpur",
        division: "Khulna"
      },
      {
        upazila: "Narail Sadar",
        district: "Narail",
        division: "Khulna"
      },
      {
        upazila: "Lohagara",
        district: "Narail",
        division: "Khulna"
      },
      {
        upazila: "Kalia",
        district: "Narail",
        division: "Khulna"
      },
      {
        upazila: "Satkhira Sadar",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Assasuni",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Debhata",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Tala",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Kalaroa",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Kaliganj",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Shyamnagar",
        district: "Satkhira",
        division: "Khulna"
      },
      {
        upazila: "Adamdighi",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Bogra Sadar",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Sherpur",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Dhunat",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Dhupchanchia",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Gabtali",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Kahaloo",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Nandigram",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Sahajanpur",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Sariakandi",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Shibganj",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Sonatala",
        district: "Bogra",
        division: "Rajshahi"
      },
      {
        upazila: "Joypurhat Sadar",
        district: "Joypurhat",
        division: "Rajshahi"
      },
      {
        upazila: "Akkelpur",
        district: "Joypurhat",
        division: "Rajshahi"
      },
      {
        upazila: "Kalai",
        district: "Joypurhat",
        division: "Rajshahi"
      },
      {
        upazila: "Khetlal",
        district: "Joypurhat",
        division: "Rajshahi"
      },
      {
        upazila: "Panchbibi",
        district: "Joypurhat",
        division: "Rajshahi"
      },
      {
        upazila: "Naogaon Sadar",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Mohadevpur",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Manda",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Niamatpur",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Atrai",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Raninagar",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Patnitala",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Dhamoirhat",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Sapahar",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Porsha",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Badalgachhi",
        district: "Naogaon",
        division: "Rajshahi"
      },
      {
        upazila: "Natore Sadar",
        district: "Natore",
        division: "Rajshahi"
      },
      {
        upazila: "Baraigram",
        district: "Natore",
        division: "Rajshahi"
      },
      {
        upazila: "Bagatipara",
        district: "Natore",
        division: "Rajshahi"
      },
      {
        upazila: "Lalpur",
        district: "Natore",
        division: "Rajshahi"
      },
      {
        upazila: "Gurudaspur",
        district: "Natore",
        division: "Rajshahi"
      },
      {
        upazila: "Singra",
        district: "Natore",
        division: "Rajshahi"
      },
      {
        upazila: "Bholahat",
        district: "Chapainawabganj",
        division: "Rajshahi"
      },
      {
        upazila: "Gomastapur",
        district: "Chapainawabganj",
        division: "Rajshahi"
      },
      {
        upazila: "Nachole",
        district: "Chapainawabganj",
        division: "Rajshahi"
      },
      {
        upazila: "Chapainawabganj Sadar",
        district: "Chapainawabganj",
        division: "Rajshahi"
      },
      {
        upazila: "Shibganj",
        district: "Chapainawabganj",
        division: "Rajshahi"
      },
      {
        upazila: "Atgharia",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Bera",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Bhangura",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Chatmohar",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Faridpur",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Ishwardi",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Pabna Sadar",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Santhia",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Sujanagar",
        district: "Pabna",
        division: "Rajshahi"
      },
      {
        upazila: "Bagha",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Bagmara",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Charghat",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Durgapur",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Godagari",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Mohanpur",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Paba",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Puthia",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Tanore",
        district: "Rajshahi",
        division: "Rajshahi"
      },
      {
        upazila: "Sirajganj Sadar",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Belkuchi",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Chauhali",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Kamarkhanda",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Kazipur",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Raiganj",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Shahjadpur",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Tarash",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Ullahpara",
        district: "Sirajganj",
        division: "Rajshahi"
      },
      {
        upazila: "Birampur",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Birganj",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Biral",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Bochaganj",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Chirirbandar",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Phulbari",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Ghoraghat",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Hakimpur",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Kaharole",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Khansama",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Dinajpur Sadar",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Nawabganj",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Parbatipur",
        district: "Dinajpur",
        division: "Rangpur"
      },
      {
        upazila: "Fulchhari",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Gaibandha Sadar",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Gobindaganj",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Palashbari",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Sadullapur",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Saghata",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Sundarganj",
        district: "Gaibandha",
        division: "Rangpur"
      },
      {
        upazila: "Kurigram Sadar",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Nageshwari",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Bhurungamari",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Phulbari",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Rajarhat",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Ulipur",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Chilmari",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Rowmari",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Char Rajibpur",
        district: "Kurigram",
        division: "Rangpur"
      },
      {
        upazila: "Lalmonirhat Sadar",
        district: "Lalmonirhat",
        division: "Rangpur"
      },
      {
        upazila: "Aditmari",
        district: "Lalmonirhat",
        division: "Rangpur"
      },
      {
        upazila: "Kaliganj",
        district: "Lalmonirhat",
        division: "Rangpur"
      },
      {
        upazila: "Hatibandha",
        district: "Lalmonirhat",
        division: "Rangpur"
      },
      {
        upazila: "Patgram",
        district: "Lalmonirhat",
        division: "Rangpur"
      },
      {
        upazila: "Nilphamari Sadar",
        district: "Nilphamari",
        division: "Rangpur"
      },
      {
        upazila: "Saidpur",
        district: "Nilphamari",
        division: "Rangpur"
      },
      {
        upazila: "Jaldhaka",
        district: "Nilphamari",
        division: "Rangpur"
      },
      {
        upazila: "Kishoreganj",
        district: "Nilphamari",
        division: "Rangpur"
      },
      {
        upazila: "Domar",
        district: "Nilphamari",
        division: "Rangpur"
      },
      {
        upazila: "Dimla",
        district: "Nilphamari",
        division: "Rangpur"
      },
      {
        upazila: "Panchagarh Sadar",
        district: "Panchagarh",
        division: "Rangpur"
      },
      {
        upazila: "Debiganj",
        district: "Panchagarh",
        division: "Rangpur"
      },
      {
        upazila: "Boda",
        district: "Panchagarh",
        division: "Rangpur"
      },
      {
        upazila: "Atwari",
        district: "Panchagarh",
        division: "Rangpur"
      },
      {
        upazila: "Tetulia",
        district: "Panchagarh",
        division: "Rangpur"
      },
      {
        upazila: "Badarganj",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Mithapukur",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Gangachara",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Kaunia",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Rangpur Sadar",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Pirgachha",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Pirganj",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Taraganj",
        district: "Rangpur",
        division: "Rangpur"
      },
      {
        upazila: "Thakurgaon Sadar",
        district: "Thakurgaon",
        division: "Rangpur"
      },
      {
        upazila: "Pirganj",
        district: "Thakurgaon",
        division: "Rangpur"
      },
      {
        upazila: "Baliadangi",
        district: "Thakurgaon",
        division: "Rangpur"
      },
      {
        upazila: "Haripur",
        district: "Thakurgaon",
        division: "Rangpur"
      },
      {
        upazila: "Ranisankail",
        district: "Thakurgaon",
        division: "Rangpur"
      },
      {
        upazila: "Ajmiriganj",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Baniachang",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Bahubal",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Chunarughat",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Habiganj Sadar",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Lakhai",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Madhabpur",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Nabiganj",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Shaistagonj",
        district: "Habiganj",
        division: "Sylhet"
      },
      {
        upazila: "Moulvibazar Sadar",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Barlekha",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Juri",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Kamalganj",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Kulaura",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Rajnagar",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Sreemangal",
        district: "Moulvibazar",
        division: "Sylhet"
      },
      {
        upazila: "Bishwamvarpur",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Chhatak",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Derai",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Dharampasha",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Dowarabazar",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Jagannathpur",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Jamalganj",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Sulla",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Sunamganj Sadar",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Shanthiganj",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Tahirpur",
        district: "Sunamganj",
        division: "Sylhet"
      },
      {
        upazila: "Sylhet Sadar",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Beanibazar",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Bishwanath",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Dakshin Surma",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Balaganj",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Companiganj",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Fenchuganj",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Golapganj",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Gowainghat",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Jaintiapur",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Kanaighat",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Zakiganj",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Osmaninagar",
        district: "Sylhet",
        division: "Sylhet"
      },
      {
        upazila: "Eidgaon",
        district: "Cox's Bazar",
        division: "Chattogram"
      },
      {
        upazila: "Madhyanagar",
        district: "Sunamganj",
        division: "Sylhet"
      }
    ];
  }
});

// node_modules/@bangladeshi/bangladesh-address/build/src/division/division.js
var require_division = __commonJS({
  "node_modules/@bangladeshi/bangladesh-address/build/src/division/division.js"(exports) {
    "use strict";
    var __importDefault = exports && exports.__importDefault || function(mod) {
      return mod && mod.__esModule ? mod : { "default": mod };
    };
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.getDivision = void 0;
    var bd_upazila_json_1 = __importDefault(require_bd_upazila());
    var getDivision = (name) => {
      const normalizedName = String(name).toLowerCase();
      return bd_upazila_json_1.default.filter((item) => item.division.toLowerCase() === normalizedName);
    };
    exports.getDivision = getDivision;
  }
});

// node_modules/@bangladeshi/bangladesh-address/build/src/district/index.js
var require_district = __commonJS({
  "node_modules/@bangladeshi/bangladesh-address/build/src/district/index.js"(exports) {
    var __importDefault = exports && exports.__importDefault || function(mod) {
      return mod && mod.__esModule ? mod : { "default": mod };
    };
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.getDivisionOfDistrict = exports.isValidDistrict = exports.allDistricts = exports.districtsOf = void 0;
    var bd_upazila_json_1 = __importDefault(require_bd_upazila());
    var division_1 = require_division();
    var districtNameSet = new Set(bd_upazila_json_1.default.map((item) => item.district.toLowerCase()));
    var districtToDivisionMap = /* @__PURE__ */ new Map();
    bd_upazila_json_1.default.forEach((item) => {
      districtToDivisionMap.set(item.district.toLowerCase(), item.division);
    });
    var districtsOf = (division) => {
      const filteredDivision = (0, division_1.getDivision)(division);
      const districts = /* @__PURE__ */ new Set();
      filteredDivision.forEach((item) => districts.add(item.district));
      return Array.from(districts);
    };
    exports.districtsOf = districtsOf;
    var allDistricts = () => {
      const districts = /* @__PURE__ */ new Set();
      bd_upazila_json_1.default.forEach((item) => districts.add(item.district));
      return Array.from(districts);
    };
    exports.allDistricts = allDistricts;
    var isValidDistrict = (name) => {
      if (!name || typeof name !== "string") {
        return false;
      }
      return districtNameSet.has(name.trim().toLowerCase());
    };
    exports.isValidDistrict = isValidDistrict;
    var getDivisionOfDistrict = (district) => {
      if (!district || typeof district !== "string") {
        return void 0;
      }
      return districtToDivisionMap.get(district.trim().toLowerCase());
    };
    exports.getDivisionOfDistrict = getDivisionOfDistrict;
  }
});

export {
  require_district
};
//# sourceMappingURL=chunk-U7PKUSJ4.js.map
