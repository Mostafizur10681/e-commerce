    ...
    <div v-else>
      <transition appear name="fade">
        <div class="grid md:grid-cols-2 gap-8">
          <!-- Image -->
          <div class="relative">
            <img :src="product.image" :alt="product.name" class="w-full h-auto rounded-lg shadow-sm" />
          </div>
          <!-- Details -->
          <div class="flex flex-col space-y-4">
            ...
            <button @click="addToCart" class="mt-4 bg-primary hover:bg-primary-dark text-white py-2 px-4 rounded-lg flex items-center gap-2 w-max transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 sticky bottom-4 z-10 md:static"
      ...
      <section class="mt-12 overflow-hidden">
        ...
        <div class="mt-10 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800 rounded-xl p-6">
          ...
          <form @submit.prevent="submitReview" class="grid gap-4 max-w-lg transition-all duration-300">
            <div>
            ...
            <div>
              <select v-model.number="newReview.rating" required class="w-full p-3 border dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 focus:ring-2 focus:ring-primary focus:outline-none transition-all">
...
function addToCart() {
  ...
  toast.show('Successfully product added');
}

function submitReview() {
  if (!newReview.value.name || !newReview.value.rating || !newReview.value.comment) return;
