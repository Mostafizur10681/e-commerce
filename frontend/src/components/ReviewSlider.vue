<template>
  <section class="py-14 px-4 bg-gray-50 dark:bg-gray-900/50" data-aos="fade-up">
    <div class="max-w-7xl mx-auto">
      <h2 class="section-title">What Our Customers Say</h2>
      <div class="relative overflow-hidden">
        <div class="flex transition-transform duration-500 ease-in-out" :style="{ transform: `translateX(-${active * 100}%)` }">
          <div v-for="review in reviews" :key="review.id"
            class="w-full flex-shrink-0 px-2 sm:px-4">
            <div class="card p-6 sm:p-8 max-w-2xl mx-auto text-center">
              <img :src="review.avatar" :alt="review.name" class="w-16 h-16 rounded-full mx-auto mb-4 ring-4 ring-primary/20 object-cover" />
              <!-- Stars -->
              <div class="flex justify-center gap-1 mb-4">
                <svg v-for="n in 5" :key="n" class="w-5 h-5"
                  :class="n <= review.rating ? 'text-amber-400' : 'text-gray-300 dark:text-gray-600'"
                  fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
              </div>
              <p class="text-gray-600 dark:text-gray-300 italic mb-4 text-sm sm:text-base leading-relaxed">"{{ review.text }}"</p>
              <h4 class="font-bold text-gray-800 dark:text-gray-100">{{ review.name }}</h4>
              <p class="text-xs text-gray-400">{{ review.date }}</p>
            </div>
          </div>
        </div>
        <!-- Dots -->
        <div class="flex justify-center gap-2 mt-6">
          <button v-for="(review, i) in reviews" :key="review.id" @click="goTo(i)"
            class="w-2.5 h-2.5 rounded-full transition-all duration-300"
            :class="active === i ? 'bg-primary w-8' : 'bg-gray-300 dark:bg-gray-600 hover:bg-primary/50'">
          </button>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup>
import { ref, onMounted, onBeforeUnmount } from 'vue';
import reviews from '../data/reviews.json';

const active = ref(0);
let timer = null;

const next = () => { active.value = (active.value + 1) % reviews.length; };
const goTo = (i) => { active.value = i; resetTimer(); };
const resetTimer = () => { clearInterval(timer); timer = setInterval(next, 6000); };

onMounted(() => { timer = setInterval(next, 6000); });
onBeforeUnmount(() => { clearInterval(timer); });
</script>
