      ...
      <div v-if="uiStore.isCartDrawerOpen"
        ...
        <div class="flex-1 overflow-y-auto p-4">
          ...
          <div v-else class="space-y-4">
            <div v-for="item in cartItems" :key="item.id || item.product.id" class="flex gap-4 items-center bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg">
              ...
              <div class="flex-1">
                ...
                <div v-if="item.selectedAttributes && Object.keys(item.selectedAttributes).length > 0" class="text-xs text-gray-500 mt-1 flex flex-wrap gap-1">
                  <span v-for="(val, name) in item.selectedAttributes" :key="name" class="bg-white dark:bg-gray-800 px-1.5 py-0.5 rounded border border-gray-100 dark:border-gray-700 capitalize">
                    {{ name }}: {{ val }}
                  </span>
                </div>
                ...
                <div class="flex items-center mt-2 border border-gray-200 dark:border-gray-700 rounded-md w-max bg-white dark:bg-gray-800">
                  <button @click="decrease(item.id || item.product.id)" class="px-2.5 py-0.5 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition">-</button>
                  <span class="px-3 text-sm font-medium">{{ item.quantity }}</span>
                  <button @click="increase(item.id || item.product.id)" class="px-2.5 py-0.5 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition">+</button>
                </div>
              </div>
              <button @click="remove(item.id || item.product.id)" class="p-2 text-gray-400 hover:text-red-500 transition rounded-full hover:bg-red-50 dark:hover:bg-red-900/20">
